...
:: 环境检查
where git >nul 2>&1 || (
    echo [错误] Git未安装或未加入PATH
	
    call :FAIL "Git工具缺失"
)

if not exist "%repo_path%\.git" (
    echo [错误] 无效的Qt仓库：%repo_path%
...
if %FAILED_COUNT% gtr 0 (
    echo [警告] 完成 - 存在失败的patch应用
    exit /b 1
) else if %SUCCESS_COUNT% equ 0 (
    echo [警告] 完成 - 没有成功应用的patch
    exit /b 1
) else (
    echo [√] 所有patch应用成功
    exit /b 0
...
echo.
echo ============================================
echo [%TIME%] 操作失败: 错误代码 %errorlevel% 
...
if not exist "%module_path%\.git" (
    echo [警告] 跳过无效模块：%module_path%
	set /a "SKIPPED_COUNT+=1"
    goto :EOF
)

echo.
echo [信息] 应用到模块：%module_path%
echo        Patch文件：%~nx2

pushd "%module_path%"
git apply --check "%patch_file%" || (
    popd
    echo [警告] 跳过不兼容patch：%~nx2
	set /a "SKIPPED_COUNT+=1"
    goto :EOF
)
